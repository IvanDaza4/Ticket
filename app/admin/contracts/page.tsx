...
import { Plus, Search, FileText, Calendar, Clock, DollarSign, Building2, ChevronRight } from 'lucide-react'
...
export default function ContractsPage() {
  ...
  const [newContract, setNewContract] = useState({
    organization_id: '', name: '', plan: 'bronze' as PlanTier,
    start_date: new Date().toISOString().split('T')[0], end_date: '',
    monthly_hours: 20, monthly_fee: 0, hourly_rate: 0,
  })
  ...
  async function fetchData() {
    const [contractsRes, orgsRes] = await Promise.all([
      supabase.from('contracts').select('*, organization:organizations(name)').order('start_date', { ascending: false }),
  ...
  async function createContract() {
    const { error } = await supabase.from('contracts').insert([{ ...newContract, end_date: newContract.end_date || null }])
    if (!error) {
      setIsCreateOpen(false)
      setNewContract({ organization_id: '', name: '', plan: 'bronze', start_date: new Date().toISOString().split('T')[0], end_date: '', monthly_hours: 20, monthly_fee: 0, hourly_rate: 0 })
      fetchData()
    }
  }
  ...
  const stats = {
    ...
    totalMonthlyRevenue: contracts.filter(c => c.is_active).reduce((sum, c) => sum + (c.monthly_fee || 0), 0),
  }
    ...
    <div className="p-4 md:p-6 lg:p-8 space-y-6 md:space-y-8 animate-fade-in">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm md:text-base">Gestiona los contratos de servicio</p>
        </div>
        <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
          ...
          <DialogContent className="sm:max-w-[500px]">
            <DialogHeader>
              ...
              <DialogDescription>Define los terminos del contrato de servicio.</DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                <Label>Cliente *</Label>
                <Select value={newContract.organization_id} onValueChange={v => setNewContract({ ...newContract, organization_id: v })}>
              ...
              <div className="grid gap-2">
                ...
                <Input value={newContract.name} onChange={e => setNewContract({ ...newContract, name: e.target.value })} placeholder="Soporte IT 2024" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-2">
                  <Label>Plan</Label>
                  <Select value={newContract.plan} onValueChange={(v: PlanTier) => setNewContract({ ...newContract, plan: v })}>
                ...
                <div className="grid gap-2">
                  ...
                  <Input type="number" value={newContract.monthly_hours} onChange={e => setNewContract({ ...newContract, monthly_hours: parseInt(e.target.value) || 0 })} />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-2">
                  ...
                  <Input type="number" step="0.01" value={newContract.monthly_fee} onChange={e => setNewContract({ ...newContract, monthly_fee: parseFloat(e.target.value) || 0 })} />
                </div>
                <div className="grid gap-2">
                  <Label>Tarifa/Hora (EUR)</Label>
                  <Input type="number" step="0.01" value={newContract.hourly_rate} onChange={e => setNewContract({ ...newContract, hourly_rate: parseFloat(e.target.value) || 0 })} />
                </div>
              </div>
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setIsCreateOpen(false)}>Cancelar</Button>
              <Button onClick={createContract} disabled={!newContract.organization_id || !newContract.name || !newContract.start_date}>
                Crear Contrato
              </Button>
            </DialogFooter>
          </DialogContent>
      ...
      <div className="grid grid-cols-2 gap-3 md:gap-4 md:grid-cols-4">
        ...
        <Card className="py-3 md:py-6 gap-2 md:gap-6">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 md:pb-2 px-3 md:px-6">
            <CardTitle className="text-xs md:text-sm font-medium">Bronze</CardTitle>
        ...
        <Card className="py-3 md:py-6 gap-2 md:gap-6">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 md:pb-2 px-3 md:px-6">
            <CardTitle className="text-xs md:text-sm font-medium">Silver</CardTitle>
        ...
        <Card className="py-3 md:py-6 gap-2 md:gap-6">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 md:pb-2 px-3 md:px-6">
            <CardTitle className="text-xs md:text-sm font-medium">Ingresos/Mes</CardTitle>
            <DollarSign className="h-3.5 w-3.5 md:h-4 md:w-4 text-green-600" />
          </CardHeader>
          <CardContent className="px-3 md:px-6">
            <div className="text-xl md:text-2xl font-bold">
              {stats.totalMonthlyRevenue.toLocaleString('es-ES', { style: 'currency', currency: 'EUR' })}
            </div>
          </CardContent>
        </Card>
      </div>
        ...
        <CardHeader>
          <div className="flex items-center gap-3">
            <div className="relative flex-1 max-w-sm">
              ...
              <Input placeholder="Buscar contratos..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-9" />
            </div>
          </div>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-muted-foreground text-sm">{searchQuery ? 'No se encontraron resultados' : 'Crea tu primer contrato para empezar'}</p>
            </div>
          ) : (
            <>
              {/* Mobile Cards */}
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <p className="font-semibold text-sm text-foreground truncate">{contract.name}</p>
                    ...
                    <div className="grid grid-cols-3 gap-2 text-center">
                      <div className="bg-muted/40 rounded-lg p-2">
                        ...
                        <Badge variant="outline" className={`text-xs mt-0.5 ${planColors[contract.plan]}`}>
                          {contract.plan.charAt(0).toUpperCase() + contract.plan.slice(1)}
                        </Badge>
                      </div>
                      <div className="bg-muted/40 rounded-lg p-2">
                        <p className="text-xs text-muted-foreground">Horas/Mes</p>
                        <p className="text-sm font-bold">{contract.monthly_hours || 0}h</p>
                      </div>
                      <div className="bg-muted/40 rounded-lg p-2">
                    ...
                    {/* Date */}
                    <div className="flex items-center gap-1 text-xs text-muted-foreground border-t border-border/40 pt-2">
                  ...
                  <TableBody>
                    {filteredContracts.map(contract => (
                      <TableRow key={contract.id}>
                        <TableCell>
                          ...
                          {contract.monthly_hours && (
                            <div className="text-sm text-muted-foreground flex items-center gap-1">
                              <Clock className="h-3 w-3" />{contract.monthly_hours} horas/mes
                            </div>
                          )}
                        </TableCell>
                        ...
                        <TableCell className="text-right">
                          <Button variant="ghost" size="sm" onClick={() => window.location.href = `/admin/contracts/${contract.id}`}>
                            Ver detalles
                          </Button>
                        </TableCell>
                      </TableRow>
